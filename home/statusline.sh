...
#   RIGHT: <model> <effort> <think> | ctx%/size | $cost | 5h% 7d%
#
...
[ -n "$EFFORT" ] && RIGHT+=" ${DIM}${EFFORT}${RESET}"
...
if [ -n "$CTXPCT" ]; then
  ...
  [ -n "$CTXSIZE" ] && RIGHT+="${DIM}/$(hum "$CTXSIZE")${RESET}"
fi
...
if [ -n "$H5$D7" ]; then
  RIGHT+=" ${DIM}|${RESET}"
  [ -n "$H5" ] && RIGHT+=" ${DIM}5h:${RESET}${GREEN}$(printf '%.0f' "$H5")%${RESET}"
  [ -n "$D7" ] && RIGHT+=" ${DIM}7d:${RESET}${GREEN}$(printf '%.0f' "$D7")%${RESET}"
fi
...
if [ "$WIDTH" -gt 0 ] && [ "$PAD" -ge 1 ]; then
  if [ -n "$BADGE" ] && [ "$PAD" -ge $(( BV + 2 )) ]; then
