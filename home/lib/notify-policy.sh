...
policy_discontinuity(){ local now="$1" dir="$2" old watermark; mkdir -p "$dir"; old="$(cat "$dir/last-observed-clock" 2>/dev/null || true)"; watermark="$(cat "$dir/watermark" 2>/dev/null || true)"; printf '%s\n' "$now" > "$dir/last-observed-clock"; if [[ "$old" =~ ^[0-9]+$ ]] && [ "$((old-now))" -gt 5 ]; then [ -n "$watermark" ] && printf '%s\n' "$watermark" > "$dir/watermark"; printf 'declare\n'; return 2; fi; printf 'ok\n'; }
