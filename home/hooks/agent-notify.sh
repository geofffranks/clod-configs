...
# Lib dependencies: the shared identity library (sole formatter for the alert
# title and the canonical [source:type] body tag — the hook calls its
# functions) and the credential-free mac Notification Center lane
# (notify_mac_available / notify_mac_send). notify-send.sh is deliberately NOT
# sourced — the push is inline curl below.
