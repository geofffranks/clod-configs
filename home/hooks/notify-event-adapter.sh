...
process(){
  local line="$1" emitted emitted_epoch id typ trans reason key decision envelope
  ...
  case "$typ:$trans:$reason" in turn:*:*|provider:*:*|ask_user_question:*:*|interrogative:*:*|approval_pending:plan_handoff:*|approval_pending:goal_proposal:*) ;; *) log "decision=silent reason=unsupported-event family=$typ"; return;; esac
