...
set -euo pipefail
input=$(cat)
if ! jq -e 'type == "object" and .tool_name == "Grep" and (.tool_input | type == "object") and (.tool_input.pattern | type == "string") and ((.tool_input.path | type == "string") or (.tool_input.path | type == "array" and all(.[]; type == "string"))) and (.tool_input.max_results | type == "number" and floor == .)' >/dev/null 2>&1 <<<"$input"; then exit 0; fi
