...
run_adapter(){ local dir="$1" now="$2" frame="$3"; mkdir -p "$dir"; printf '%s' '{"port":1,"session_id":"sid","credential_file_path":""}' > "$dir/startup.json"; printf '#!/usr/bin/env bash\nprintf '\''%%s\\n'\'' %q\n' "$frame" > "$dir/curl"; chmod +x "$dir/curl"; PATH="$dir:$PATH" AGENT_NOTIFY_ADAPTER_LOG_DIR="$dir/logs" AGENT_NOTIFY_TEST_NOW="$now" bash "$ADAPTER" "$dir/startup.json" >/dev/null 2>&1; }
